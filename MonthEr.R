m1=rep(0,12)
data<-read.table("D:\\data_xijiang\\F_F_1988\\rout\\1988real_runoff.txt")
n1 = nrow(data)
n2 = 1
ave_data <- 0
for(i in 1:n1){
...
month_R <- c(1:12)
lines(month_R,m1,lty =1)
write.table(m1,"D:\\data_xijiang\\F_F_1982\\rout\\AVE_Y_O.txt",col.names = F,row.names = F,quote = F)
data_Month_M <- read.table("D:\\data_xijiang\\F_F_1988\\rout\\result_GY\\GY   .month")[,3]
lines(month_R,data_Month_M,lty=2)
legend(4,140,legend = c("模拟径流","实测径流"),col = c("black","black"),lty = c(1,2),lwd = 2)
write.table(data_Month_M,"D:\\data_xijiang\\F_F_1982\\rout\\AVE_Y_M.txt",col.names = F,row.names = F,quote = F)
